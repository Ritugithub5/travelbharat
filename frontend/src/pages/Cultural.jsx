...
export default function CulturalGuidePage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-slate-900 font-sans pt-20 sm:pt-24 pb-24 antialiased selection:bg-amber-100">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6 mb-12 border-b border-slate-200/80 pb-8">
          ...
          <div className="w-full md:w-80">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder="Search art form, location, or category..." 
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-10 pr-10 py-2.5 text-sm bg-white border-2 border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:border-amber-400 shadow-sm transition"
              />
              {searchTerm && (
                <button
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-4 scrollbar-none mb-12 border-b border-slate-200/60">
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-xl text-xs font-medium transition-all duration-300 whitespace-nowrap ${
        ...
        <div className="space-y-12">
          {filteredItems.length === 0 ? (
            <div className="text-center py-16 text-slate-400 font-light text-sm bg-white rounded-3xl border border-slate-200">
              ...
              <p className="text-lg font-medium text-slate-600">No art forms found</p>
              <p className="text-sm text-slate-400 mt-1">Try adjusting your search or filter</p>
              <button
                onClick={() => { setSearchTerm(''); setSelectedCategory('All'); }}
                className="mt-4 text-amber-600 hover:text-amber-700 font-medium text-sm inline-flex items-center gap-1"
              >
